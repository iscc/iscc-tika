...
# by checking if the current directory ends with iscc-tika/bindings/iscc-tika-python
current_dir=$(pwd)
...
if [[ $current_dir =~ iscc-tika/bindings/iscc-tika-python$ ]]; then
    ...
    cd /workspace/bindings/iscc-tika-python

    for PYBIN in /opt/python/cp38*/bin; do
        "${PYBIN}/pip" install maturin
        "${PYBIN}/pip" install wheel
        #"${PYBIN}/maturin" build -i "${PYBIN}/python" --release --out /workspace/bindings/iscc-tika-python/dist --compatibility manylinux_2_34
        "${PYBIN}/maturin" build --release -i "${PYBIN}/python" --out /workspace/bindings/iscc-tika-python/target/wheels --compatibility manylinux_2_28
    done

else
    echo "Please make sure to run the script from iscc-tika/bindings/iscc-tika-python or inside docker"
    exit 1
fi