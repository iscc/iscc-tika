...
fn main() {
    ...
    //let python_bind_dir = fs::canonicalize("../bindings/iscc-tika-python/python/iscc_tika").unwrap();
    ...
    if let Some(parent_dir) = out_dir.parent().and_then(|p| p.parent()) {
        ...
        if let Ok(entries) = fs::read_dir(parent_dir) {
            for entry in entries.flatten() {
                ...
                if path.is_dir() {
                    // Get the directory name as a string and check if it starts with "iscc-tika-"
                    if let Some(dir_name) = path.file_name().and_then(|name| name.to_str()) {
                        if dir_name.starts_with("iscc-tika-") {
